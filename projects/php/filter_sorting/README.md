...
The Advanced Search Page provides users with more refined search options. The fields for this form will include:

- a field for keywords
- a dropdown list of categories to search for that keyword in (i.e. the table's columns)
- a field of checkboxes that allow the user to search for one or more specific publishers
- fields that allow the user to search between specific years
- how to order the results (ascending or descending)

Depending upon which options the user does (or doesn't) choose, the SQL statement to retrieve their records may change. You will need to construct it clause by clause. 

After the form is submitted, it must retain its values (i.e. have prepopulated form fields).


## Search Results

The search results will include a table below the advanced search form that lists all of the results for the user's query. 

If there is an error or no returned results, an error message will be displayed below the advanced search form instead. 

If there is no query string, the table will not be displayed.
